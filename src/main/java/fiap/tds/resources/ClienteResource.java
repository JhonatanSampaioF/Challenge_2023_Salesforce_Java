...
public class ClienteResource {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response updateCliente(@PathParam("id_clie") int id_clie, Cliente cliente) {
        Cliente existingCliente = clienteRepo.findById(id_clie);

        if (existingCliente == null) {
            return Response.status(Response.Status.NOT_FOUND).entity("Cliente não encontrado").build();
        }

        // Atualiza todos os campos do cliente existente com os valores fornecidos no objeto cliente
        existingCliente.setNome(cliente.getNome());
        existingCliente.setSobrenome(cliente.getSobrenome());
        existingCliente.setEmail(cliente.getEmail());
        existingCliente.setEmpresa(cliente.getEmpresa());
        existingCliente.setTamanhoEmpresa(cliente.getTamanhoEmpresa());
        ...
        clienteRepo.update(existingCliente);

        return Response.status(Response.Status.OK).entity(existingCliente).build();
    }*/
